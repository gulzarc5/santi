...
$sql_count = "SELECT * FROM `users` WHERE  `user_type` = '3'";
...
if (!empty($request['search']['value'])) {
	...
	if ($res = $connection->query($sql)) {
		...
		while ($row_user = $res->fetch_assoc()) {
			$action = '<a href="employee_edit_form.php?u_id='.$row_user['id'].'" class="btn btn-success">Edit</a>';
             if ($row_user['status'] == 1) {
             	$action = $action.'<a href="php/employee/status_update.php?u_id='.$row_user['id'].'&status=2" class="btn btn-danger">Deactivate</a>';
             }elseif ($row_user['status'] == 2) {
             	$action = $action.'<a href="php/employee/status_update.php?u_id='.$row_user['id'].'&status=1" class="btn btn-success">Activate</a>';
			}
			$action = $action.'<a href="php/employee/reset_password.php?u_id='.$row_user['id'].'" class="btn btn-danger">Reset Password</a>';
